...
;; Revision History
;; ----------------
;; 8/9/95	- moved out of project-manager into twist-down system.
;; 7/21/1995	- file created
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defmethod view-shrink-to-fit-subviews ((view view) &optional (border 10))
  (when (subviews view)
    (set-view-size view (view-fit-subviews-size view border))))
...
(defclass list-view-mixin ()
  ((vsize :initarg :vsize :initform 18 :accessor vsize)
  ...
  (let (lastview vpos)
    ...
    (if lastview       
      (setq vpos (+ (1- (the fixnum (point-v (view-position lastview))))
                    (the fixnum (vsize view))))
      (setq vpos (vsize view)))
...
(defmethod remove-view-from-list ((view list-view-mixin) v)
  ...
  (let ((after-views (views-after v view)))
    ...
    (when (null (view-list view))
      (set-view-size view (make-point (point-h (view-size view)) (vsize view)))
      (list-view-adjust-siblings-and-parent view))))